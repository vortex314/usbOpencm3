...
static bool usb_serial_need_empty_tx = false; // after sending a full packet , we need to send a 0 size
/* Buffer to be used for control requests. */
uint8_t usbd_control_buffer[128];
void *__dso_handle;

#include <deque>
std::deque<uint8_t> rxd;
std::deque<uint8_t> txd;
#define MAX 1024
...
{
	uint32_t freeSpace = MAX - txd.size();
	if (freeSpace < 4)
		return;
	if (freeSpace < (bufLength + 2)) // space free < demand
	{
		txd.push_back('*');
	}
	else
...
static void cdcacm_sof_callback(void)
{
	...
	for (int i = 0; i < len; i++)
	{
		usb_serial_tx_buf[i] = txd.front();
		txd.pop_front();
	}
	uint16_t sent = usbd_ep_write_packet(usbd_dev, 0x82, usb_serial_tx_buf, len);
...
int main(void)
{
	...
	logSomething.start();
	mainThread.start();
	xTaskCreate(BlinkerTask, "Blinker", 200, NULL, configMAX_PRIORITIES - 1, (xTaskHandle *)NULL);
	xTaskCreate(UsbTask, "Usb", 200, NULL, configMAX_PRIORITIES - 1, (xTaskHandle *)NULL);
	vTaskStartScheduler();
}
